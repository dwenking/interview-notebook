...
链表是通过指针串联在一起的线性结构，每个节点由数据和指针两部分构成。链表和数组的对比：

* 链表：增删复杂度 $O(1)$，查询复杂度 $O(n)$。适用于数据量不固定，频繁增删，较少查询的情景。
* 数组：增删复杂度 $O(n)$，查询复杂度 $O(1)$。适用于数据量固定，频繁查询，较少增删的情景。
...
复杂度分析：时间复杂度 $O(2^n)$，遍历每个元素取/不取的情况。空间复杂度 $O(n)$，递归深度为n。
...
复杂度分析：时间复杂度 $O(n!)$。空间复杂度 $O(n)$，递归深度为n。
...
贪心的思想就是**通过局部最优达到全局最优**。贪心的题经常需要排序。

#### 参考习题

| 题目                                                        | 提示                                                         |
| ----------------------------------------------------------- | ------------------------------------------------------------ |
| [最大子数组和](https://leetcode.cn/problems/maximum-subarray/) | 经典贪心问题，如果当前和为负数的话就舍弃当前和并重新开始统计 |
| [分发糖果](https://leetcode.cn/problems/candy/)                | 难点在于想到遍历方式                                         |
